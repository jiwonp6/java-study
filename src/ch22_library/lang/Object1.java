...
public class Object1 {
    public static void main(String[] args) {
        ...
        // 타입 정보를 조회 : 객체의 정확한 타입을 학인 가능
        // 런타임 도중에 다른 클래스의 메소드나, 필드, 생성자 등에 접근해서
        // 객체 생성, 메소드 호출, 필드 변경 가능 => Reflection

        // 스레드와 관련되어 있는 메소드
        object.notify();            // 객체를 기다리는 스레드 중 하나를 깨움
        object.notifyAll();         // 객체를 기다리는 모든 스레드 깨움
        // object.wait();           // 현재 스레드 일시 중단하고 깨울 때까지 기다림.
        // object.wait(1000);       // 스레드 중단하고 n밀리초 기다림
        // object.wait(100, 100);   // 스레드 중단하고 n밀리초, m나노초 기다림

        // object.clone();          // 객체의 복제본을 생성하는 메소드
        // object.finalize();       // 더 이상 사용하지 않는 메소드
                                    // => Garbage Collector 에 의해 객체가 삭제되기 전에 호출되는 메소드
