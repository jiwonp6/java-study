...
public class Ex3 {
    /*
        ...
        요구 사항
        - 로그 파일 형식
            : "chat.log" 로그 파일은 각 줄마다 하나의 메시지를 기록하며,
               각 메시지는 다음 형식을 따릅니다: [시간] 사용자명: 메시지
        - 결과 파일
            : "chatSummary.txt" 파일에는 각 사용자별로 보낸 메시지의 총 개수를 기록합니다.
    ...
    public static void main(String[] args) {
        /*
            ...
            // 유저이름을 key 로 하고 횟수를 value 로 하는 Map (기본값 0, 사용자명을 키로 만날때마다 +1씩 카운트)
            userMessageMap.put(userName, userMessageMap.getOrDefault(userName, 0) + 1);
         */

        try {
            List<Employee> employeeList = new ArrayList<>(
                    List.of(new Employee("박진수"),
                            new Employee("길동씨"),
                            new Employee("무대리"),
                            new Employee("강감찬"))
            );

            BufferedWriter bw = new BufferedWriter(new FileWriter("src/ch29_io_stream/files/chatSummary.txt"));
            BufferedReader br = new BufferedReader(new FileReader("src/ch29_io_stream/files/chat.log"));
            ...
            while (true) {
                ...
                for (int i = 0; i < employeeList.size(); i++) {
                    if (line.contains(employeeList.get(i).name)){
                        employeeList.get(i).messageNum++;
                    }
                }
            }
            // 출력
            for (int i = 0; i < employeeList.size(); i++) {
                bw.write(employeeList.get(i).name + ": " + employeeList.get(i).messageNum);
                bw.newLine();
                System.out.println(employeeList.get(i).name + ": " + employeeList.get(i).messageNum);
            }
    ...
    static class Employee {
        ...
        public Employee(String name, int messageNum) {
            this.name = name;
