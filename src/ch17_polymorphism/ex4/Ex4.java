...
public class Ex4 {
    /*
        ...
        요구사항:
        Cookie라는 추상 클래스를 만듭니다. 이 클래스는 run()과 jump()라는 메서드를 포함해야 하며,
        이 메서드들은 캐릭터의 기본 동작을 정의합니다.
        Gingerbread, StrawberryCookie, ChocoCookie 클래스를 만들고, 이들 각각이 Cookie 클래스를 상속받도록 합니다.
        각 클래스는 run()과 jump() 메서드를 자신의 캐릭터 스타일에 맞게 구현해야 합니다.
        또한, 각 캐릭터는 고유의 특수 능력을 하나씩 가질 수 있습니다(예: specialSkill() 메서드).
